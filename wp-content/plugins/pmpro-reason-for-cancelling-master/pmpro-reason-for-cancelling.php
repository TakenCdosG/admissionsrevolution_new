...
function pmpror4c_init() {
    if (!empty($_REQUEST['confirm']) && !empty($_REQUEST['membership_cancel']) && !isset($_REQUEST['reason-checkbox'])) {
...
function pmpror4c_pmpro_email_body($body, $email) {
    ...
    $reasoncheckbox = $_REQUEST['reason-checkbox'];
    if (!empty($reasoncheckbox)) {
        $reason_checkbox_str = "";
        foreach ($reasoncheckbox as $rc) {
            $reason_checkbox_str = $reason_checkbox_str . ", " . $rc;
        }
        //replace in standard templates
        if ($email->template == "cancel" || $email->template == "cancel_admin") {
            $body = str_replace("has been cancelled.</p>", "has been cancelled.</p><p>Reason: " . $reason_checkbox_str . "</p> <p>Comments: " . $reason . "</p>", $body);
        }
